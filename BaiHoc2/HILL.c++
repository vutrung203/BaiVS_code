...
string encrypt(string message, vector<vector<int>> key) {
    ...
    // If message length is not divisible by key size, pad with 'X'
    ...
    for (int i = 0; i < message.length(); i += keySize) {
        string block = message.substr(i, keySize);
        vector<int> encryptedBlock(keySize, 0);

        // Calculate the encrypted block
        for (int j = 0; j < keySize; j++) {
            for (int k = 0; k < keySize; k++) {
                encryptedBlock[j] += (key[j][k] * (block[k] - 'A'));
            }
            encryptedBlock[j] %= 26;
        }

        // Convert back to characters and append to encrypted text
        for (int j = 0; j < keySize; j++) {
            encryptedText += (encryptedBlock[j] + 'A');
        }
    }
...
int main() {
    ...
    cout << "Enter the key matrix (row-wise):" << endl;
